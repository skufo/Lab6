function   frame = readFrame(nomefile, format, k, bit)
...
%
%   frame = readFrame(nomefile, format, k, bit)
%   In this case the last variable is a variable that can be 0 or 1
%   0 is used for 8 bit depth frame input
%   1 is used for 16 bit depth frame input
%   In this case alle the parameters shall be specified otherwise it
%   automatically set 8 bit depth frame inputs
%
%   Examples:
...
if ~exist('bit','var'), bit=0; end

[rows cols color bytes_per_frame] = getInfo(nomefile,format, bit);

byte_shift = bytes_per_frame * k;
if fseek(fid,byte_shift,'bof')<0,
    error('Impossible to get frame %d in file %s',k,nomefile);
end
frame (:,:,1) = transpose(fread(fid, [cols rows], 'uint8'));
if color>0,
...
if numel(frame)<(rows*cols*needed_components)
    ...
    function [rows cols color bytes_per_frame cstring] = getInfo(nomefile,format, bit)
        ...
        if isfield(format,'color');
            switch lower(format.color)
                case {400, '400'}
                    ...
                    else error('Input element bit unknown');
                    end
                    end
                    color = 0;
                    cstring = '.y';
                case {420, '420'}
                    ...
                    if bit==0
                        bytes_per_frame = rows * cols * 1.5;
                            ...
                            bytes_per_frame = rows * cols * 1.5 * 2;
                    else error('Input element bit unknown');
                    end
                    end
                    cstring = '.yuv';
